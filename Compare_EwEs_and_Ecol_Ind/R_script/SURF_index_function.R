...
# Function to calculate SURF index

SURF_index_function <- function(EwE_period, data_raw, select_species) {
  ...
  # SURF index calculation
  
  sum_square_value <- Df_clean %>%
    ...
    pivot_longer(cols = everything(),
                 ...
                 values_to = "diet_prop") %>%
    filter(diet_prop > 0) %>%
    mutate(square_value = diet_prop^2) %>%
    select(square_value) %>%
    pull() %>%
    sum()
  
  Surf_index <- sum_square_value/Total_number_paths
  
  assign(paste0(EwE_period, "_SURF_Index_", select_species), Surf_index, envir = .GlobalEnv)
  
  for_print <- c(select_species, Surf_index)
