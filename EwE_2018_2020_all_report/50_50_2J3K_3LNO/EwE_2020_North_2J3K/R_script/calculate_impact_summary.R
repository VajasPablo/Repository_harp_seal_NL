...
calculate_impact_summary <- function(LoD_rename, F_data, species_unselect, species, total_number_group, species_short) {
  
  df_group_impacted <- function(LoD_rename, F_data, species_unselect){
    ...
    LoD_0 <- rep(0, nrow(Df_percentage))
  ...
  summary_function <- function(df_change, total_number_group, threshold) {
    df_change %>%
      ...
      bind_rows(
        data.frame(t(round(colSums(df_change == "+"), 2)), Description = "Count Prop. +"),
        data.frame(t(round(colSums(df_change == "-"), 2)), Description = "Count Prop. -")
      ) %>%
  ...
  result_df <- df_group_impacted(LoD_rename, F_data, species_unselect)
  ...
  #print(knitr::kable(table_20, caption = paste0("For ", species, " Table of the species number and species percentage impacted by more than 20% of total change"), align = "c"))
  #print(knitr::kable(table_40, caption = paste0("For ", species, " Table of the species number and species percentage impacted by more than 40% of total change"), align = "c"))
...
assign(object_name_DT, DT_tables, envir = .GlobalEnv)
  ...
  # save raw data 
  files_in_dir <- list.files(pattern = "\\.Rproj$")
  files_in_dir <- gsub("\\.Rproj$", "", files_in_dir)
  
  write.xlsx(result_df, file = here("R_results_export", paste0(files_in_dir,"_", species_short, "_raw_data_percent_change", ".xlsx")), rowNames = FALSE)
