combined_simulation_generate_function <- function(simulation_names, Weight, Group_no, Type, time_step_start, nb_time_steps, 
                                                  fill_value_min, fill_value_max, output_file, species_1, species_1_number,
                                                  F_species_1, species_2, species_2_number, F_species_2, order_colonnes, ...) {
  biomass_sim <- FALSE
  if (!missing(...)) {
    biomass_sim <- TRUE
  }
  ...
  add_obs_column <- function(simulation_data, species_1, species_1_number, F_species_1, species_2, species_2_number, F_species_2, order_colonnes, biomass_sim) {
    
    if (biomass_sim) {
      species_1_init <- as.character(c(paste0("B", species_1, "_obs"), 1, species_1_number, -1))
      species_2_init <- as.character(c(paste0("B", species_2, "_obs"), 1, species_2_number, -1))
    } else {
  ...
  simulation_data <- add_obs_column(simulation_data, species_1, species_1_number, F_species_1, species_2, species_2_number, F_species_2, order_colonnes, biomass_sim)
