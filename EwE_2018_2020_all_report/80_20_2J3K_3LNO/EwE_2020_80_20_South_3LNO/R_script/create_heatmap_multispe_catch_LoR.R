...
### Function to create heatmap for explore other species catch in multispecific simulation ###

create_heatmap_multispe_catch_LoR <- function(data, SPECIE_NAME, species_one_fixe, species_two_fixe, species_one, species_two) {
  
  data_clean_0 <- data %>%
    mutate_at(vars(starts_with("catch")), as.numeric) %>%
    filter(rowSums(select(., starts_with("catch")), na.rm = TRUE) > 0)
  
  data_process <- data_clean_0 %>% 
    ...
    mutate(
      ...
      !!paste0("LoR_", species_one) := factor(get(paste0("LoR_", species_one)), levels = c("100", "75", "50", "obs", "25", "1"))) %>%
    mutate(
  ...
  absolute_heatmap <- ggplot(data_process, aes_string(x = paste0("LoR_", species_two), y = paste0("LoR_", species_one), fill = "value" )) +
    ...
    labs(title = paste("Heatmap", SPECIE_NAME, "catch ~ LoR", species_two, "x LoR", species_one),
  ...
  relative_heatmap <- ggplot(data_process, aes_string(x = paste0("LoR_", species_two), y = paste0("LoR_", species_one), fill = "Value_normalized" )) +
    ...
    labs(title = paste("Heatmap", SPECIE_NAME, "relative catch ~ LoR", species_two, "x LoR", species_one),
         x = paste("Level of Restoration", species_two),
         y = paste("Level of Restoration", species_one),
         fill = paste(SPECIE_NAME, "\nrelative catch")) +   
    theme_minimal() + 
