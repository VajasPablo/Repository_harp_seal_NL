...
heatmap_multispecific_group_impacted_LoR <- function(data, species_one_fixe, species_two_fixe, species_one, species_two, percent) {
  ...
  for (i in 3:8) {  # Boucle sur les colonnes 3 à 8
    ggplot_list[[i-2]] <- ggplot(data_process, aes_string(x = paste0("LoR_", species_two), y = paste0("LoR_", species_one), fill = paste0("`", names(data_process)[i], "`"))) +
      ...
      labs(x = paste("Level of Restoration", species_two),
