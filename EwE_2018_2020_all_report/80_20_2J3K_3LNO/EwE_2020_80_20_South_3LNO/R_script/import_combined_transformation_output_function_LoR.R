...
import_combined_transformation_output_function_LoR <- function(input_select, species_short, species_short_2, folder, wich_simulation, F_vector, F_vector_2, obs_F_vector, obs_F_vector_2, digit_import_value, digit_import_value_2, LoD_vector_name, change_name_for_LoD, print) {
  ...
  if (wich_simulation == "final") {
    F_vector_2 <- trimws(as.character(format(round(F_vector_2, digits = digit_import_value_2), nsmall = digit_import_value_2)))
  } else {
    F_vector_2
  }
  
  obs_F_vector <-  trimws(as.character(format(obs_F_vector, digits = digit_import_value, nsmall = digit_import_value))) %>% 
    unlist() %>% 
    unname()
  obs_F_vector_2 <-  trimws(as.character(format(obs_F_vector_2, digits = digit_import_value_2, nsmall = digit_import_value_2))) %>% 
    unlist() %>% 
    unname()
  ...
  file_names1 <- paste0(input_select, "_annual_", "B", species_short, "_", F_vector)
  file_names2 <- paste0("B", species_short_2, "_", F_vector_2)
  ...
  file_names1 <- paste0(input_select, "_annual_", "LoR_", species_short, "_", LoD_vector_name)
  file_names2 <- paste0("LoR_", species_short_2, "_", LoD_vector_name)
  file_names_expand <- expand.grid(file_names1 = file_names1, file_names2 = file_names2)
  file_names_final <- file_names_expand %>% 
    mutate(final_name = paste0(file_names1, "_" ,file_names2)) %>% 
    select(final_name) %>% unname() %>% unlist()
  ...
  assign(paste0("Df_merged_", species_short, "_vs_", species_short_2, "_", input_select, "_", wich_simulation, "_LoR"), Df_merged, envir = .GlobalEnv)
