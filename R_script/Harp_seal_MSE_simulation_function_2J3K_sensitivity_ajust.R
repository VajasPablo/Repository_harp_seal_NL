...
Harp_seal_MSE_simulation_function_2J3K_sensitivity_ajust <- function(prop_sensitivity, region, data_simulation, title){
  ...
  # Traitement des données de simulation
  data_process <- data_simulation %>%
    ...
    mutate(seal_biomass_change = if_else(
      LoR_capelin == "obs" & LoR_cod == "obs",
      0,
      round((biomass_seal[LoR_capelin == "obs" & LoR_cod == "obs"] - biomass_seal) /
              biomass_seal[LoR_capelin == "obs" & LoR_cod == "obs"], 5) * -100
    )) %>%
    ...
    mutate(
      LoR_cod = paste0("LoR_Cod_", LoR_cod),
      LoR_capelin = paste0("LoR_Capelin_", LoR_capelin),
      Harp_seal_total_biomass_2J3K = biomass_seal * 237600
    ) %>%
    mutate(Year = 2035) %>%
    mutate(
  ...
  # Ajustement des valeurs de Harp_seal_total_biomass_2J3K pour correspondre à la normalisation
  data_process <- data_process %>%
    mutate(Harp_seal_total_biomass_2J3K = Harp_seal_total_biomass_2J3K / max_value_stock_assessment)
  ...
  gg <- ggplot(data_assessment, aes(x = Year, y = Biomass_T_area_rescale)) +
    geom_line() +
    labs(x = "Year", y = "Biomass (relative to 2020)") +
    ...
    geom_segment(aes(x = 2020, y = 1, xend = 2035, yend = 1.00), color = "#1f77b4", size = 0.05, show.legend = FALSE) +
    geom_segment(aes(x = 2020, y = 1, xend = 2035, yend = 0.958), color = "#2ca02c", size = 0.05, show.legend = FALSE) +
    geom_segment(aes(x = 2020, y = 1, xend = 2035, yend = 0.971), color = "#d62728", size = 0.05, show.legend = FALSE) +
    geom_segment(aes(x = 2020, y = 1, xend = 2035, yend = 0.980), color = "#ff7f0e", size = 0.05, show.legend = FALSE) +
    geom_segment(aes(x = 2020, y = 1, xend = 2035, yend = 1.12), color = "#1f77b4", size = 0.05, show.legend = FALSE) +
    geom_segment(aes(x = 2020, y = 1, xend = 2035, yend = 1.28), color = "#9467bd", size = 0.05, show.legend = FALSE) +
    geom_segment(aes(x = 2020, y = 1, xend = 2035, yend = 1.48), color = "#e377c2", size = 0.05, show.legend = FALSE) 
