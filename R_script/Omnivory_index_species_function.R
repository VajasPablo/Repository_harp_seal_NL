...
Omnivory_index_species_function <- function(EwE_period, data_raw, select_species) {
  ...
  OI_sp_index <- Df_clean %>% select("Name","Omnivory index") %>% 
    filter(Name %in% select_species) %>% 
    select("Omnivory index") %>% pull() %>% as.numeric()
  
  # save in globalenv
  assign(paste0(EwE_period, "_Omnivory_index_", select_species), OI_sp_index, envir = .GlobalEnv)
  
  for_print <- c(select_species,OI_sp_index)
