...
# Trophic level by species select 

Trophic_level_species_function <- function(EwE_period, data_raw, select_species) {
  ...
  TL_sp_index <- Df_clean %>% select("Group name","Trophic level") %>% 
    filter(`Group name` %in% select_species) %>% 
    select("Trophic level") %>% pull() %>% as.numeric()
  
  # save in globalenv
  assign(paste0(EwE_period, "_Trophic_Level_", select_species), TL_sp_index, envir = .GlobalEnv)
  
  for_print <- c(select_species,TL_sp_index)
