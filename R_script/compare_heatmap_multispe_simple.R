...
compare_heatmap_multispe_simple <- function(data_period_1, period_1, data_period_2, period_2, species_facet_wrap){
  
  df_1 <- data_period_1 %>%
    rename(!!paste0("Value_", period_1) := value) %>%
    mutate(!!paste0("Value_", period_1) := case_when(
      get(paste0("Value_", period_1)) == "0" ~ "0",
      get(paste0("Value_", period_1)) == "-" ~ "0",
  ...
  ggplot(df_merged, aes(x = LoD_seal, y = Group_name, fill = Change)) +
    ...
    labs(x = NULL, y = NULL, fill = NULL, title = paste0("Difference between highest value of \nEwE ", period_1, " & EwE ", period_2)) +
    coord_fixed() +
