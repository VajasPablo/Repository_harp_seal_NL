...
heatmap_multispecific_biomass_restoration_function_3LNO_Sens <- function(data, species_number, species_one_fixe, species_two_fixe, species_one, species_two, target_species, period, obs_dens, hist_val) {
  ...
  data_process <- data %>%
    ...
    mutate(!!paste0("LoD_", species_two) := factor(get(paste0("LoD_", species_two)), levels = c("0", "25", "obs", "50", "75", "99")),
